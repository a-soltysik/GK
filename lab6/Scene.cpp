...
auto Scene::startup() -> void
{
    ...
    const auto multiplier = Vector3f {0.5f, 0.9f, 1.f};
    const auto white        = Vector3f {1.f, 1.f, 1.f};
    createLight(white, multiplier, {0.f, 0.f, 10.f});
...
{
    const auto symbol = utils::KeyboardHandler::instance().keys.find(key);
    if (symbol == utils::KeyboardHandler::instance().keys.end())
    {
        return;
    }

    if (symbol->second >= '1' && symbol->second <= '8')
    {
        const auto it = lights.find(static_cast<uint32_t>(symbol->second - '0' - 1));
        if (it != lights.cend())
        {
            state = LightCam {
                &it->second,
